...
import { validateCourse, isIntInRange, sanitizeText, sanitizeFixed, validateMember, validateGradeInput } from "./validate.js";
import { validateSheetInput, validateSlotsInput, validateSlotPatch } from "./validate.js";
...
// ----------------- Courses & Members (your existing endpoints) -----------------
// (keep your current 1.a–1.f code here unchanged)
...
// ---------- helpers for sheets ----------
function findSheetInCourses(courses, sheetId) {
  for (const c of courses) {
    const sheet = (c.sheets || []).find(s => s.id === Number(sheetId));
    if (sheet) return { course: c, sheet });
  }
  return {};
}
...
app.get("/api/slots/:id/members", async (req, res) => {
  ...
  // Find slot in SHEETS_FILE first
  const sheets = await readJSON(SHEETS_FILE, []);
  let foundSheet = null;
  let foundSlot  = null;
  for (const sh of sheets) {
  ...
  // Get member details from matching course (in COURSES_FILE) using the sheet's term/section
  const courses = await readJSON(COURSES_FILE, []);
  ...
  // Find the slot (and its sheet) in sheets.json
  const sheets = await readJSON(SHEETS_FILE, []);
  let sheet = null, slot = null;
  for (const sh of sheets) {
    const found = (sh.slots || []).find(s => Number(s.id) === slotId);
    if (found) { sheet = sh; slot = found; break; }
  }
  if (!slot) return res.status(404).json({ ok:false, error:"Slot not found." });

  // Use the sheet's term/section to load members from courses.json
  const courses = await readJSON(COURSES_FILE, []);
  const course = courses.find(c => c.term === sheet.term && c.section === sheet.section);
  const memIndex = new Map((course?.members || []).map(m => [String(m.id), m]));

  const members = (slot.members || []).map(id => memIndex.get(String(id))).filter(Boolean);
  res.json({ ok:true, members });
});
}
  ...
  for (const c of courses) {
    for (const sheet of (c.sheets || [])) {
      ...
      if (slot) {
        const memIndex = new Map((c.members||[]).map(m => [String(m.id), m]));
        const members = (slot.members || []).map(id => memIndex.get(String(id))).filter(Boolean);
...
app.post("/api/grades", async (req, res) => {
  ...
  // Grades live on the sheet -> use SHEETS_FILE
  const sheets = await readJSON(SHEETS_FILE, []);
...
if (!ok) return res.status(400).json({ ok:false, errors });
  ...
  const courses = await readJSON(COURSES_FILE, []);
  const { sheet } = findSheetInCourses(courses, value.sheetId);
  if (!sheet) return res.status(404).json({ ok:false, error:"Sheet not found." });
